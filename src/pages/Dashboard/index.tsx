...
import PlayerScroller from "../../components/playerScroller/PlayerScroller";
...
export default function Dashboard() {
  ...
  >([["carregando..."], ["carregando...", undefined], {wins: {total: 0}, played: {total:0}, loses: {total: 0}, draws: {total: 0}}, [[1, 2]], "carregando..."]);
  const { user } = useAuth();
      ...
      <div className="flex flex-col lg:flex-row w-11/12 h-[90%] lg:w-11/22 lg:h-3/4">
        <div className="flex flex-col space-y-2 h-full w-full shrink-0 lg:space-y-0 lg:flex-row lg:mr-5 lg:w-[73%] narrow:w-2/3">
          <div className="flex h-fit w-full lg:w-fit lg:h-full shrink">
            <TeamData games={(dashboardData[2] as IGames).played.total} wins={(dashboardData[2] as IGames).wins.total} loses={(dashboardData[2] as IGames).loses.total} draws={(dashboardData[2] as IGames).draws.total} topFormation={dashboardData[4] as string} />
          </div>
